...
class Net:
    ...
    def __init__(self, optimizer_g: optim, optimizer_d: optim, criterion, generator, discriminator, dataset: Union[Dataset, AggregateDataset]):
        """
        ...
        if gpu:
            self.generator = self.generator.to(device='cuda')
            self.discriminator = self.discriminator.to(device='cuda')
    ...
    def train(self, epochs: int, patience: int, verbosity_interval: int = 1):
        ...
        real_labels = torch.full((labels_size_d,), 1, dtype=torch.float, device=device)
        fake_labels = torch.full((labels_size_d,), 0, dtype=torch.float, device=device)
        ...
        for epoch in range(1, epochs + 1):
            ...
            with torch.set_grad_enabled(True):
                ...
                noise = torch.randn(labels_size_d, x_train.shape[1], 1, device=device)
                ...
                error_d = error_real_d + error_fake_d
                ...
                error_g = self.criterion(output_g, real_labels)
                error_g.backward()
                d_g2 = output_g.mean().item()
                self.optimizer_g.step()

            self.hist[epoch - 1] = np.array([error_d.item(), error_g.cpu().item()])
            ...
            # logging losses
            writer.add_scalar('Error/generator', error_d.item(), epoch)
            writer.add_scalar('Error/discriminator', error_g.item(), epoch)
            if epoch == 1 or epoch % verbosity_interval == 0:
                print(f"Epoch {epoch}, Generator Error: {error_g.item()}, " +
                      f"Discriminator Error: {error_g.item()}, Dx: {d_x}, Dg1: {d_g1}, Dg2: {d_g2}")
        # self.load()
