...
class RNN(Module):
    ...
    def forward(self, input_data: float, hidden):
        """
        ...
        output, hidden = self.gru(self.expand_dims(input_data), hidden)
    ...
    def expand_dims(self, value):
        """expands the dims for the generator"""
        value = value.squeeze()
        if torch.is_tensor(value) and value.ndim == 1:
            return torch.unsqueeze(torch.unsqueeze(value.squeeze(), axis=1), axis=2).to(self.device)
        return torch.unsqueeze(torch.unsqueeze(torch.tensor([value]), axis=1), axis=2).to(
            self.device
        )
